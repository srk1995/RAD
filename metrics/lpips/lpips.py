...
class MyDataset(Dataset):
    ...
    def __len__(self):
        return len(self.im0_path)

    def __getitem__(self, idx):
        self.im0 = np.array(Image.open(self.im0_path[idx]).convert("RGB"))/255
        self.im1_list = glob(f"{self.im1_path}/*{self.mask}*{os.path.basename(self.im0_path[idx]).split('.')[0]}*")
        self.im1 = [(self.transform(np.array(Image.open(x).convert("RGB")) / 255) - 0.5) * 2 for x in self.im1_list]
        return [(self.transform(self.im0) - 0.5) * 2] * len(self.im1), self.im1


if __name__ == "__main__":
    loss_fn = lpips.LPIPS(pretrained=True, net='alex')
    im0_path = '/home/srk1995/PycharmProjects/diffusers/test_images'
    im1_path = '/home/srk1995/PycharmProjects/diffusers/Inpainting images'
    mask_list = glob(f"{im1_path}/mask/*")
    mask_list = [os.path.basename(x).split('_')[1] for x in mask_list]
    mask_list = set(mask_list)
    for mask in mask_list:
        dataset = MyDataset(im0_path, im1_path, mask,
                            transform=transforms.Compose([transforms.Resize((256, 256)), transforms.ToTensor()]))
        dataloader = DataLoader(dataset, batch_size=1, shuffle=True)
        for i, x in enumerate(dataloader):
            im0, im1 = x
            im0, im1 = torch.concat(im0, dim=0).to(torch.float), torch.concat(im1, dim=0).to(torch.float)
            loss = loss_fn(im0, im1).mean()
            print(f"Mask type: {mask}, lpips: {loss} ")
            break