...
class ResnetBlock2D(nn.Module):
        ...
        *,
        ...
        if temb_channels is not None:
            if self.time_embedding_norm == "default":
                self.time_emb_proj = nn.Conv2d(temb_channels, out_channels, 1)
            elif self.time_embedding_norm == "scale_shift":
                self.time_emb_proj = nn.Conv2d(temb_channels, 2 * out_channels, 1)
            elif self.time_embedding_norm == "global":
    ...
    def forward(self, input_tensor: torch.FloatTensor, temb: torch.FloatTensor, *args, **kwargs) -> torch.FloatTensor:
        ...
        if self.time_emb_proj is not None:
            ...
            if self.time_embedding_norm == "global":
                temb = self.time_emb_proj(temb)[:, :, None, None]
            else:
                temb = nn.UpsamplingBilinear2d(size=hidden_states.shape[2:])(temb)
                temb = self.time_emb_proj(temb) #[:, :, None, None]
