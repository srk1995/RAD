        ...
        if from_flax:
            ...
            if low_cpu_mem_usage:
                ...
                for k, v in state_dict.items():
                    if ('time') in k:
                        if 'weight' in k or 'W' in k:
                            v_shape = v.shape
                            if len(v_shape) ==1:
                                conv_weight[k] = v.view(v_shape[0], 1, 1, 1)
                            else:
                                conv_weight[k] = v.view(v_shape[0], v_shape[1], 1, 1)

                        else:
                            conv_weight[k] = v
                state_dict = {k: v for k, v in state_dict.items() if 'time_emb' not in k}
                state_dict.update(conv_weight)
