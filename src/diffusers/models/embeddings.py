...
def get_timestep_embedding_local(
    timesteps: torch.Tensor,
    embedding_dim: int,
    flip_sin_to_cos: bool = False,
    downscale_freq_shift: float = 1,
    ...
    """
    assert len(timesteps.shape) == 4, "Timesteps should be a 1d-array"
    ...
    emb = torch.exp(exponent)
    emb = timesteps.float() * emb[None, :, None, None]
...
class LocalTimestepEmbedding(nn.Module):
    def __init__(
        self,
        in_channels: int,
        time_embed_dim: int,
        ...
        self.linear_1 = nn.Conv2d(in_channels, time_embed_dim, 1)
        ...
        if out_dim is not None:
            ...
            time_embed_dim_out = time_embed_dim
        self.linear_2 = nn.Conv2d(time_embed_dim, time_embed_dim_out, 1)
...
class LocalTimesteps(nn.Module):
    def __init__(self, num_channels: int, flip_sin_to_cos: bool, downscale_freq_shift: float, num_timesteps: int):
        super().__init__()
    ...
    def forward(self, timesteps):
        t_emb = get_timestep_embedding_local(
            timesteps,
            self.num_channels,
            flip_sin_to_cos=self.flip_sin_to_cos,
            downscale_freq_shift=self.downscale_freq_shift,
            scale=1,
            max_period=torch.pi / self.num_timesteps if self.num_timesteps != 1 else 10000,
        )
        return t_emb
...
class GaussianFourierProjection_local(nn.Module):
    """Gaussian Fourier embeddings for noise levels."""
    ...
    def __init__(
        ...
        super().__init__()
        shape = torch.randn(embedding_size, 1, 1, 1)
        self.weight = nn.Parameter(shape)
        self.log = log
        self.flip_sin_to_cos = flip_sin_to_cos
    ...
    def forward(self, x):
        ...
        x_proj = F.conv2d(x, self.weight) * 2 * np.pi
