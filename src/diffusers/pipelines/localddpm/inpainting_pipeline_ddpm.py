...
class InPaintDDPMPipeline(DiffusionPipeline,
    TextualInversionLoaderMixin,
    StableDiffusionLoraLoaderMixin,
    IPAdapterMixin,
    PeftAdapterMixin,
    ...
    def __call__(
        ...
        >>> pipe = InPaintLocalDDPMPipeline.from_pretrained("google/ddpm-cat-256")
        ...
        image = torch.randn_like(self.clean_image) * (self.mask) + self.clean_image * (1 - self.mask)
        ...
        timestep = (
            torch.linspace(self.scheduler.num_train_timesteps - 10, 0, num_inference_steps)
            .round()
            .to(torch.int64)
        )
