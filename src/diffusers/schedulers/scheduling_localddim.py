...
class LocalDDIMSchedulerOutput(BaseOutput):
    """
...
class LocalDDIMScheduler(SchedulerMixin, ConfigMixin):
    """
    ...
    def _get_variance(self, timestep, prev_timestep):
        ...
        elif (timestep[0] >= self.mask_steps) and (prev_timestep[0] < self.mask_steps):
            alpha_prod_t_prev = 1 - self.nu * self.mask
        else:
            self.set_variables(self.mask, prev_timestep)
            alpha_prod_t_prev = self.a_t_bar
        self.set_variables(self.mask, timestep)

        beta_prod_t = 1 - alpha_prod_t
        beta_prod_t_prev = 1 - alpha_prod_t_prev
        variance = (beta_prod_t_prev / beta_prod_t.clamp(min=1e-20)) * (1 - (alpha_prod_t / alpha_prod_t_prev.clamp(min=1e-20).clamp(max=1)))
    ...
    def step(
    ...
    ) -> Union[LocalDDIMSchedulerOutput, Tuple]:
        """
        ...
        variance = (beta_prod_t_prev / beta_prod_t.clamp(min=1e-20)) * (
                    1 - (alpha_prod_t / alpha_prod_t_prev.clamp(min=1e-20)).clamp(max=1))
        ...
        prev_sample = torch.where(current_beta_t == 0, sample, prev_sample)
        ...
        return LocalDDIMSchedulerOutput(prev_sample=prev_sample, pred_original_sample=pred_original_sample)
